...
def find_contacts(message):
    ...
    return True if result else False #чтоб не предлагал менять контакт если не найдет ID контакт

def start_app():
    while True:
        user_choice = view.show_mein_menu() #вернет int от 1 до 8
        match user_choice: #работает в данном случае как if
            case 1:
            ...
            case 7:
                if find_contacts(text.input_id_for_delete):
